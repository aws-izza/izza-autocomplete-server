                    ...
                    script {
                        withCredentials([usernamePassword(credentialsId: "${GIT_CREDENTIALS}", usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                            sh '''
                                ...
                                grep "image: " environments/app/autocomplete-server/autocomplete-server.yaml || echo "No image line found"
                                
                                # 이미지 태그 업데이트
                                sed -i "s|image: ${ECR_REGISTRY}/${IMAGE_REPOSITORY}:.*|image: ${ECR_REGISTRY}/${IMAGE_REPOSITORY}:${IMAGE_TAG}|g" environments/app/autocomplete-server/autocomplete-server.yaml
                                
                                # 변경사항 확인
                                echo "Updated image in GitOps repo:"
                                grep "image: " environments/app/autocomplete-server/autocomplete-server.yaml
                                ...
                                if [ -n "$(git diff --name-only)" ]; then
                                    # 커밋 및 푸시
                                    git add environments/app/autocomplete-server/autocomplete-server.yaml
